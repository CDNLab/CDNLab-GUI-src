...
#include "HttpCDNBrowserBase.h"

HttpCDNBrowserBase::HttpCDNBrowserBase()
: HttpNodeBase()
{
...
void HttpCDNBrowserBase::initialize(int stage)
{
    if (stage==0)
    {
        ll = par("logLevel");

        EV_DEBUG << "Initializing base HTTP CDN browser component -- phase 0\n";
        ...
        controller = dynamic_cast<CDNController*>(getParentModule()->getParentModule()->getSubmodule("controller"));
        if (controller == NULL)
        ...
        //location info
        locationInfo.IP_address_on_web=this->getParentModule()->par("IP_address_on_web").stringValue();
        locationInfo.latitude=this->getParentModule()->par("latitude").doubleValue();
        locationInfo.longitude=this->getParentModule()->par("longitude").doubleValue();
        locationInfo.country=this->getParentModule()->par("country").stringValue();
        locationInfo.city=this->getParentModule()->par("city").stringValue();

        eventTimer = new cMessage("eventTimer");

        //record response times
        responseTimes.setName("responseTime");
        hopCounts.setName("hopCounts");
        hopCountsHistogram.setName("hopCountsHistogram");

    }
    else if (stage==1)
    {
        EV_DEBUG << "Initializing base HTTP CDN browser component -- phase 1\n";
...
void HttpCDNBrowserBase::finish()
{
    EV_SUMMARY << "\n-----------------------------------------------------------\n";
    ...
    EV_SUMMARY << "Total bytes received " << totalBytesReceived << "\n";
...
void HttpCDNBrowserBase::handleSelfMessages(cMessage *msg)
{
...
void HttpCDNBrowserBase::handleSelfActivityStart()
{
...
void HttpCDNBrowserBase::handleSelfStartSession()
{
...
void HttpCDNBrowserBase::handleSelfNextMessage()
{
...
void HttpCDNBrowserBase::handleSelfScriptedEvent()
{
...
void HttpCDNBrowserBase::handleSelfDelayedRequestMessage(cMessage *msg)
{
...
void HttpCDNBrowserBase::handleDataMessage(cMessage *msg)
{
    ...
    if (appmsg->result()!=200 || (HttpContentType)appmsg->contentType()==CT_UNKNOWN)
    {
        ...
        if(appmsg->result()!=204)//no content
            htmlErrorsReceived++;
        delete msg;
        return;
    }
    else
    {
        ...
        /* we just assuming html pages in this version
        ...
        */
        totalBytesReceived=totalBytesReceived+appmsg->getByteLength();
        responseTimes.record(simTime() - appmsg->par("requestTimeStamp").doubleValue());
    }

    delete msg;
}

HttpRequestMessage* HttpCDNBrowserBase::generatePageRequest(std::string www, std::string pageName, bool bad, int size)
{
...
HttpRequestMessage* HttpCDNBrowserBase::generateRandomPageRequest(std::string www, bool bad, int size)
{
    EV_DEBUG << "Generating random page request, URL: " << www << endl;
    return generatePageRequest(www, "random_page.html", bad, size);
}

HttpRequestMessage* HttpCDNBrowserBase::generateResourceRequest(std::string www, std::string resource, int serial, bool bad, int size)
{
...
void HttpCDNBrowserBase::scheduleNextBrowseEvent()
{
...
void HttpCDNBrowserBase::readScriptedEvents(const char* filename)
{
    ...
    while (!std::getline(scriptfilestream, line).eof())
    {
        ...
        be.serverModule = dynamic_cast<HttpNodeBase*>(controller->getSurrogateServerModule(locationInfo, wwwpart.c_str()));
