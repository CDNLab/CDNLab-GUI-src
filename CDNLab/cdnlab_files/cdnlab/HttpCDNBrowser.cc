...
#include "HttpCDNBrowser.h"

Define_Module(HttpCDNBrowser);

HttpCDNBrowser::HttpCDNBrowser() {
    numBroken = 0;
    socketsOpened = 0;
}
...
void HttpCDNBrowser::handleMessage(cMessage *msg) {
    if (msg->isSelfMessage()) {
        ...
        if (socket == NULL) {
            ...
            EV_WARNING
                    << "No socket found for message " << msg->getName() << endl;
            delete msg;
            return;
        }
...
void HttpCDNBrowser::sendRequestToServer(BrowseEvent be) {
    int connectPort;
    char szModuleName[127];

    //if (controller->getSurrogateServerInfo(be.wwwhost.c_str(), szModuleName, connectPort) != 0)

    if (controller->selectSurrogateServer_nearest(locationInfo, be.wwwhost.c_str(), szModuleName, connectPort) != 0)
    {
        EV_ERROR << "Unable to get server info for URL " << be.wwwhost << endl;
        return;
    }

    EV_DEBUG << "Sending request to server " << be.wwwhost << " ("
                    << szModuleName << ") on port " << connectPort << endl;
    submitToSocket(szModuleName, connectPort,
            generatePageRequest(be.wwwhost, be.resourceName));
}

void HttpCDNBrowser::sendRequestToServer(HttpRequestMessage *request) {
    int connectPort;
    char szModuleName[127];

    //if (controller->getSurrogateServerInfo(request->targetUrl(), szModuleName, connectPort) != 0)
    if (controller->selectSurrogateServer_nearest(locationInfo,
            request->targetUrl(), szModuleName, connectPort) != 0) {
        EV_ERROR << "Unable to get server info for URL " << request->targetUrl()
                        << endl;
        return;
    }

    EV_DEBUG << "Sending request to server " << request->targetUrl() << " ("
                    << szModuleName << ") on port " << connectPort << endl;
    submitToSocket(szModuleName, connectPort, request);
}

void HttpCDNBrowser::sendRequestToRandomServer() {
    int connectPort;
    char szWWW[127];
    char szModuleName[127];

    if (controller->getRandomSurrogateServerInfo(szWWW, szModuleName,
            connectPort) != 0) {
        EV_ERROR << "Unable to get a random server from controller" << endl;
        return;
    }

    EV_DEBUG << "Sending request to random server " << szWWW << " ("
                    << szModuleName << ") on port " << connectPort << endl;
    submitToSocket(szModuleName, connectPort, generateRandomPageRequest(szWWW));
}

void HttpCDNBrowser::sendRequestsToServer(std::string www,
        HttpRequestQueue queue) {
    int connectPort;
    char szModuleName[127];

    if (controller->getSurrogateServerInfo(www.c_str(), szModuleName,
            connectPort) != 0) {
        EV_ERROR << "Unable to get server info for URL " << www << endl;
        return;
    }

    EV_DEBUG << "Sending requests to server " << www << " (" << szModuleName
                    << ") on port " << connectPort
                    << ". Total messages queued are " << queue.size() << endl;
    submitToSocket(szModuleName, connectPort, queue);
}

void HttpCDNBrowser::socketEstablished(int connId, void *yourPtr) {
    ...
    if (sockdata->messageQueue.empty()) {
        EV_INFO << "No data to send on socket with connection id " << connId
                       << ". Closing" << endl;
        socket->close();
        return;
    }
...
void HttpCDNBrowser::socketDataArrived(int connId, void *yourPtr, cPacket *msg,
        bool urgent) {
    EV_DEBUG << "HttpCDNBrowser: Socket data arrived on connection " << connId
                    << ": " << msg->getName() << endl;
    if (yourPtr == NULL) {
    ...
    if (--sockdata->pending == 0) {
        EV_DEBUG
                        <<
                        "Received last expected reply on this socket. Issing a close"
                        << endl;
        socket->close();
    }
...
void HttpCDNBrowser::socketStatusArrived(int connId, void *yourPtr,
        TCPStatusInfo *status) {
    // This is obviously not used at the present time.
    EV_INFO << "SOCKET STATUS ARRIVED. Socket: " << connId << endl;
}

void HttpCDNBrowser::socketPeerClosed(int connId, void *yourPtr) {
...
void HttpCDNBrowser::socketClosed(int connId, void *yourPtr) {
...
void HttpCDNBrowser::socketFailure(int connId, void *yourPtr, int code) {
...
void HttpCDNBrowser::submitToSocket(const char* moduleName, int connectPort,
        HttpRequestMessage *msg) {
    // Create a queue and push the single message
    HttpRequestQueue queue;
    //set sending time
    msg->addPar("requestTimeStamp");
    msg->par("requestTimeStamp").setDoubleValue(simTime().dbl());
