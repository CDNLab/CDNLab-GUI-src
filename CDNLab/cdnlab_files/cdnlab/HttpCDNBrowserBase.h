...
#ifndef __INET_HttpCDNBrowserBase_H
#define __INET_HttpCDNBrowserBase_H

#include "INETDefs.h"
#include "HttpNodeBase.h"
#include "controller/CDNController.h"
 ...
 * @version 1.0
 */
class INET_API HttpCDNBrowserBase: public HttpNodeBase {
protected:
    /**
...
protected:
    ...
    CDNController *controller; ///< Reference to the central controller object
    ...
    LocationInfo locationInfo; //an object holding location info of the client
    ...
    double distanceFromServer;
    long totalBytesReceived;

    cOutVector responseTimes;
    cOutVector hopCounts;
    cHistogram hopCountsHistogram;
    //@}

public:
    HttpCDNBrowserBase();
    virtual ~HttpCDNBrowserBase();
...
protected:
    ...
    virtual void sendRequestsToServer(std::string www,
            HttpRequestQueue queue) = 0;
    //@}
...
protected:
    ...
    HttpRequestMessage* generatePageRequest(std::string www, std::string page,
            bool bad = false, int size = 0);
    /** Generate a random HTTP request -- used in case we don't care which page is requested */
    HttpRequestMessage* generateRandomPageRequest(std::string www, bool bad =
            false, int size = 0);
    /** Generate a resource request, e.g. for an image or css document */
    HttpRequestMessage* generateResourceRequest(std::string www,
            std::string resource = "", int serial = 0, bool bad = false,
            int size = 0);
    //@}
