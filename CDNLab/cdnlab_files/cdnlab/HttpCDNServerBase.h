...
#ifndef __INET_HttpCDNServerBase_H
#define __INET_HttpCDNServerBase_H
 ...
 */
class INET_API HttpCDNServerBase: public HttpNodeBase {
protected:
    /**
    ...
    //custom: indicates that the message is coming from origin server
    enum TcpStatusInd {
        TCP_I_ORIGIN_DATA = 100
    };
protected:
...
protected:
    ...
    HttpReplyMessage* generateDocument(HttpRequestMessage *request,
            const char* resource, int size = 0);
    /** Generate a resource message in response to a request. */
    HttpReplyMessage* generateResourceMessage(HttpRequestMessage *request,
            std::string resource, HttpContentType category);
    ...
    //SJJ
    void registerOriginWithController();
    void registerSurrogateWithController();
    //SJJ
